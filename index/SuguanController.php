<?php
session_start();
class SuguanController extends CommonController {
    ...
    public function login(){
        ...
        $password = isset($_REQUEST['password'])?$_REQUEST['password']:"";
                        $sql = "select * from `suguan` where `gonghao` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $postData = json_decode($tmpData,true);
                        ...
                        $colum = "gonghao";
        $trues = "select * from `suguan` where `gonghao` = '".$postData[$colum]."'";
        ...
        if($result->num_rows<1){
			...
			$key = implode(',',$keyArr);
			$v = implode(',',$valArr);
                        $sql = "INSERT INTO `suguan` (`id`,".$key.") VALUES (".time().",".$v.")";
                        $result = table_sql($sql);
    ...
    public function resetPass(){
        $username = $_REQUEST['username'];
                        $counts = "select * from `suguan` where gonghao = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update suguan set mima = '123456' where gonghao = '".$username."'";
                                                                                                                                                        		        $result = table_sql($sql);
    ...
    public function page(){
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `suguan` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `suguan` (`id`,".$k.") VALUES (".time().",".$v.")";
                $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function add(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                $sql = "INSERT INTO `suguan` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE suguan SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from suguan WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `suguan` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
    ...
    public function value(){
        ...
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM suguan group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM suguan GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM suguan GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM suguan GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
