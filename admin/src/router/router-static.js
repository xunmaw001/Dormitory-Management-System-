import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import suguan from '@/views/modules/suguan/list'
    import tuisushenqing from '@/views/modules/tuisushenqing/list'
    import zhuansushenqing from '@/views/modules/zhuansushenqing/list'
    import xuesheng from '@/views/modules/xuesheng/list'
    import queqinjilu from '@/views/modules/queqinjilu/list'
    import sushegonggao from '@/views/modules/sushegonggao/list'
    import sushexinxi from '@/views/modules/sushexinxi/list'
    import sushelouhao from '@/views/modules/sushelouhao/list'
    import sushetousu from '@/views/modules/sushetousu/list'
    import ruzhushenqing from '@/views/modules/ruzhushenqing/list'
    import config from '@/views/modules/config/list'
    import sushehao from '@/views/modules/sushehao/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/suguan',
        name: '宿管',
        component: suguan
      }
      ,{
	path: '/tuisushenqing',
        name: '退宿申请',
        component: tuisushenqing
      }
      ,{
	path: '/zhuansushenqing',
        name: '转宿申请',
        component: zhuansushenqing
      }
      ,{
	path: '/xuesheng',
        name: '学生',
        component: xuesheng
      }
      ,{
	path: '/queqinjilu',
        name: '缺勤记录',
        component: queqinjilu
      }
      ,{
	path: '/sushegonggao',
        name: '宿舍公告',
        component: sushegonggao
      }
      ,{
	path: '/sushexinxi',
        name: '宿舍信息',
        component: sushexinxi
      }
      ,{
	path: '/sushelouhao',
        name: '宿舍楼号',
        component: sushelouhao
      }
      ,{
	path: '/sushetousu',
        name: '宿舍投诉',
        component: sushetousu
      }
      ,{
	path: '/ruzhushenqing',
        name: '入住申请',
        component: ruzhushenqing
      }
      ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
      ,{
	path: '/sushehao',
        name: '宿舍号',
        component: sushehao
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
