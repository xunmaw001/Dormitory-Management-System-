  ...
  <div>
    ...
    >  
     <el-row>
      ...
      <el-col :span="12">
        <el-form-item   v-if="flag=='suguan'"  label="工号" prop="gonghao">
      ...
      <el-col :span="12">
        <el-form-item   v-if="flag=='suguan'"  label="手机" prop="shouji">
          <el-input v-model="ruleForm.shouji"               placeholder="手机" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='suguan'"  label="身份证" prop="shenfenzheng">
          <el-input v-model="ruleForm.shenfenzheng"               placeholder="身份证" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='suguan'"  label="邮箱" prop="youxiang">
          <el-input v-model="ruleForm.youxiang"               placeholder="邮箱" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="24">  
        <el-form-item v-if="flag=='suguan'" label="照片" prop="zhaopian">
          <file-upload
          tip="点击上传照片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.zhaopian?ruleForm.zhaopian:''"
          @change="suguanzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item v-if="flag=='suguan'" label="楼号" prop="louhao">
          <el-select  @change="suguanlouhaoChange"  v-model="ruleForm.louhao" placeholder="请选择楼号">
            <el-option
                v-for="(item,index) in suguanlouhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  mounted() {
    ...
    this.$http({
      ...
      if (data && data.code === 0) {
        this.suguanlouhaoOptions = data.data;
      } else {
        this.$message.error(data.msg);
      }
    });
  ...
  methods: {
    ...
    suguanlouhaoChange () {
      this.$http({
        url: `follow/sushelouhao/louhao?columnValue=`+ this.ruleForm.louhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
          console.log('下2随')
    ...
    suguanzhaopianUploadChange(fileUrls) {
        this.ruleForm.zhaopian = fileUrls;
    },
    onUpdateHandler() {
      ...
      if((!this.ruleForm.gonghao)&& 'suguan'==this.flag){
      ...
      if( 'suguan' ==this.flag && this.ruleForm.shouji&&(!isMobile(this.ruleForm.shouji))){
        this.$message.error(`手机应输入手机格式`);
        return
      }
      if( 'suguan' ==this.flag && this.ruleForm.shenfenzheng&&(!checkIdCard(this.ruleForm.shenfenzheng))){
        this.$message.error(`身份证应输入身份证格式`);
        return
      }
      if( 'suguan' ==this.flag && this.ruleForm.youxiang&&(!isEmail(this.ruleForm.youxiang))){
