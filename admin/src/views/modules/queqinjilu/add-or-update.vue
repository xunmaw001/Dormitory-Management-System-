<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="缺勤日期" prop="queqinriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.queqinriqi" 
                type="date"
                :readonly="ro.queqinriqi"
                placeholder="缺勤日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.queqinriqi" label="缺勤日期" prop="queqinriqi">
              <el-input v-model="ruleForm.queqinriqi" 
                placeholder="缺勤日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chuangweihao = true;
          ...
          if(o=='queqinriqi'){
            this.ruleForm.queqinriqi = obj[o];
	    this.ro.queqinriqi = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `queqinjilu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.queqinjiluCrossAddOrUpdateFlag = false;
						 this.parent.search();
