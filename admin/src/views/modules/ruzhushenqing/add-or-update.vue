<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="申请入住时间" prop="shenqingruzhushijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.shenqingruzhushijian" 
                type="datetime"
                :readonly="ro.shenqingruzhushijian"
                placeholder="申请入住时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shenqingruzhushijian" label="申请入住时间" prop="shenqingruzhushijian">
              <el-input v-model="ruleForm.shenqingruzhushijian" 
                placeholder="申请入住时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.shenqingruzhushijian = this.getCurDateTime()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chuangweihao = true;
          ...
          if(o=='shenqingruzhushijian'){
            this.ruleForm.shenqingruzhushijian = obj[o];
	    this.ro.shenqingruzhushijian = true;
            continue;
          }
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ruzhushenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ruzhushenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
