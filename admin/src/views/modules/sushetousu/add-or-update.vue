<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="投诉时间" prop="tousushijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tousushijian" label="投诉时间" prop="tousushijian">
              <el-input v-model="ruleForm.tousushijian" 
                placeholder="投诉时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="投诉行为" prop="tousuxingwei">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.tousuxingwei" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tousuxingwei" label="投诉行为" prop="tousuxingwei">
                    <span v-html="ruleForm.tousuxingwei"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fengmian = true;
          ...
          if(o=='tousuxingwei'){
            this.ruleForm.tousuxingwei = obj[o];
	    this.ro.tousuxingwei = true;
            continue;
          }
          if(o=='tousushijian'){
    ...
    gonghaoChange () {
      this.$http({
        url: `follow/suguan/gonghao?columnValue=`+ this.ruleForm.gonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.tousuxingwei = this.ruleForm.tousuxingwei.replace(reg,'../../../phpc0l92/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `sushetousu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.sushetousuCrossAddOrUpdateFlag = false;
						 this.parent.search();
